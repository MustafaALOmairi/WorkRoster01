...
function LoggedInView() {
  ...
  const handleChangeUsername = async () => {
    ...
    try {
      ...
      if (res.ok) {
        updateUsername(trimmed); setUsernameSuccess(true); setNewUsername(""); playSound("success");
      } else {
        if (data.error === "USERNAME_TAKEN") setUsernameError(t("اسم المستخدم مستخدم بالفعل", "Username already taken"));
        else if (data.error === "USERNAME_TOO_SHORT") setUsernameError(t("اسم المستخدم قصير جداً", "Username is too short"));
        else setUsernameError(t("حدث خطأ، حاول مرة أخرى", "Something went wrong"));
        playSound("error");
      }
    } catch { setUsernameError(t("تعذر الاتصال بالخادم", "Could not connect to server")); }
    finally { setUsernameLoading(false); }
  };

  const handleChangePassword = async () => {
    setPwError(""); setPwSuccess(false);
    if (!currentPw || !newPw || !confirmPw) { setPwError(t("يرجى ملء جميع الحقول", "Please fill all fields")); return; }
    if (newPw.length < 6) { setPwError(t("كلمة المرور الجديدة يجب أن تكون 6 أحرف على الأقل", "New password must be at least 6 characters")); return; }
    if (newPw !== confirmPw) { setPwError(t("كلمتا المرور غير متطابقتين", "Passwords don't match")); return; }
    setPwLoading(true);
    if (Platform.OS !== "web") Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setPwSuccess(true); setCurrentPw(""); setNewPw(""); setConfirmPw(""); playSound("success");
      } else {
        if (data.error === "WRONG_PASSWORD") setPwError(t("كلمة المرور الحالية غير صحيحة", "Current password is incorrect"));
        else if (data.error === "PASSWORD_TOO_SHORT") setPwError(t("كلمة المرور الجديدة قصيرة جداً", "New password is too short"));
        else setPwError(t("حدث خطأ، حاول مرة أخرى", "Something went wrong"));
        playSound("error");
      }
    } catch { setPwError(t("تعذر الاتصال بالخادم", "Could not connect to server")); }
  ...
  return (
    <View style={[styles.container, { backgroundColor: bgColor, paddingTop: insets.top + webTopInset }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>{t("إدارة الحساب", "Account Management")}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView style={styles.content} contentContainerStyle={{ paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        <View style={styles.avatarSection}>
          ...
          {user?.email ? <Text style={[styles.displayEmail, { color: colors.textSecondary }]}>{user.email}</Text> : null}
          <View style={styles.syncRow}>
            <Ionicons name="cloud-done" size={14} color="#43A047" />
            <Text style={[styles.syncText, { color: "#43A047" }]}>{t("البيانات محفوظة تلقائياً", "Data auto-saved")}</Text>
          </View>
        </View>

        <Text style={[styles.sectionLabel, { color: colors.textSecondary }]}>{t("إعدادات الحساب", "Account Settings")}</Text>

        <View style={[styles.card, { backgroundColor: cardBg, borderColor }]}>
          <Pressable style={styles.menuRow} onPress={() => togglePanel("username")}>
            <View style={[styles.menuIcon, { backgroundColor: colors.accent + "18" }]}>
              <Ionicons name="person-outline" size={20} color={colors.accent} />
            </View>
            <Text style={[styles.menuLabel, { color: colors.text }]}>{t("تغيير اسم المستخدم", "Change Username")}</Text>
            <Ionicons name={panel === "username" ? "chevron-up" : "chevron-down"} size={18} color={colors.textSecondary} />
          </Pressable>
          ...
          {panel === "username" && (
            <View style={[styles.panelBody, { borderTopColor: borderColor }]}>
              ...
              <Pressable style={[styles.panelBtn, { backgroundColor: colors.accent, opacity: usernameLoading ? 0.7 : 1 }]} onPress={handleChangeUsername} disabled={usernameLoading}>
                {usernameLoading ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={styles.panelBtnText}>{t("حفظ", "Save")}</Text>}
              </Pressable>
            </View>
          )}
          ...
          <Pressable style={styles.menuRow} onPress={() => togglePanel("password")}>
            <View style={[styles.menuIcon, { backgroundColor: "#F59E0B18" }]}>
              <Ionicons name="lock-closed-outline" size={20} color="#F59E0B" />
            </View>
            <Text style={[styles.menuLabel, { color: colors.text }]}>{t("تغيير كلمة المرور", "Change Password")}</Text>
            <Ionicons name={panel === "password" ? "chevron-up" : "chevron-down"} size={18} color={colors.textSecondary} />
          </Pressable>
          ...
          {panel === "password" && (
            <View style={[styles.panelBody, { borderTopColor: borderColor }]}>
              <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor }]}>
                ...
                <TextInput style={[styles.input, { color: colors.text }]} placeholder={t("كلمة المرور الحالية", "Current password")} placeholderTextColor={colors.textSecondary} value={currentPw} onChangeText={(v) => { setCurrentPw(v); setPwError(""); setPwSuccess(false); }} secureTextEntry={!showCurPw} />
                <Pressable onPress={() => setShowCurPw(!showCurPw)} hitSlop={8}><Ionicons name={showCurPw ? "eye-off-outline" : "eye-outline"} size={18} color={colors.textSecondary} /></Pressable>
              </View>
              <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor }]}>
                <Ionicons name="key-outline" size={18} color={colors.textSecondary} />
                <TextInput style={[styles.input, { color: colors.text }]} placeholder={t("كلمة المرور الجديدة", "New password")} placeholderTextColor={colors.textSecondary} value={newPw} onChangeText={(v) => { setNewPw(v); setPwError(""); setPwSuccess(false); }} secureTextEntry={!showNewPw} />
                <Pressable onPress={() => setShowNewPw(!showNewPw)} hitSlop={8}><Ionicons name={showNewPw ? "eye-off-outline" : "eye-outline"} size={18} color={colors.textSecondary} /></Pressable>
              </View>
              <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor }]}>
                <Ionicons name="checkmark-circle-outline" size={18} color={colors.textSecondary} />
                <TextInput style={[styles.input, { color: colors.text }]} placeholder={t("تأكيد كلمة المرور الجديدة", "Confirm new password")} placeholderTextColor={colors.textSecondary} value={confirmPw} onChangeText={(v) => { setConfirmPw(v); setPwError(""); setPwSuccess(false); }} secureTextEntry={!showNewPw} />
              </View>
              ...
              <Pressable style={[styles.panelBtn, { backgroundColor: "#F59E0B", opacity: pwLoading ? 0.7 : 1 }]} onPress={handleChangePassword} disabled={pwLoading}>
                {pwLoading ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={styles.panelBtnText}>{t("حفظ", "Save")}</Text>}
              </Pressable>
            </View>
          )}
        </View>

        <Text style={[styles.sectionLabel, { color: colors.textSecondary, marginTop: 28 }]}>{t("الجلسة", "Session")}</Text>
        <Pressable onPress={handleLogout} disabled={loggingOut || deletingAccount} style={[styles.actionRow, { backgroundColor: cardBg, borderColor }]} testID="logout-btn">
          <View style={[styles.menuIcon, { backgroundColor: "#EF444418" }]}>
            {loggingOut ? <ActivityIndicator color="#EF4444" size="small" /> : <Ionicons name="log-out-outline" size={20} color="#EF4444" />}
          </View>
          <Text style={[styles.menuLabel, { color: "#EF4444" }]}>{t("تسجيل الخروج", "Sign Out")}</Text>
        </Pressable>
...
export default function AuthScreen() {
  ...
  const handleSubmit = async () => {
    setError("");
    if (!username.trim() || !password.trim()) { setError(t("يرجى ملء جميع الحقول", "Please fill all fields")); return; }
    if (!isLogin && password !== confirmPassword) { setError(t("كلمات المرور غير متطابقة", "Passwords don't match")); return; }
    if (!isLogin && password.length < 6) { setError(t("كلمة المرور يجب أن تكون 6 أحرف على الأقل", "Password must be at least 6 characters")); return; }
    if (!isLogin && username.trim().length < 3) { setError(t("اسم المستخدم يجب أن يكون 3 أحرف على الأقل", "Username must be at least 3 characters")); return; }
    if (!isLogin && email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) { setError(t("البريد الإلكتروني غير صحيح", "Invalid email address")); return; }
    ...
    if (result.ok) {
      playSound("success");
      if (isLogin) { const loaded = await loadServerData(); if (loaded) triggerReload(); }
      setLoading(false); router.back();
    } else {
      setLoading(false); playSound("error");
      const errCode = result.error || "";
      if (errCode === "INVALID_CREDENTIALS") setError(t("اسم المستخدم أو كلمة المرور غير صحيحة", "Invalid username/email or password"));
      else if (errCode === "USERNAME_TAKEN") setError(t("اسم المستخدم مستخدم بالفعل", "Username already taken"));
      else if (errCode === "EMAIL_TAKEN") setError(t("البريد الإلكتروني مستخدم بالفعل", "Email already taken"));
      else if (errCode === "INVALID_EMAIL") setError(t("البريد الإلكتروني غير صحيح", "Invalid email address"));
      else setError(t("حدث خطأ، حاول مرة أخرى", "Something went wrong, try again"));
    }
  };
        ...
        <ScrollView style={styles.content} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View style={[styles.card, { backgroundColor: cardBg, borderColor: colors.border, padding: 16, gap: 12 }]}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.border }]}>
                ...
                <TextInput
                  ...
                  placeholderTextColor={colors.textSecondary} value={username} onChangeText={setUsername}
                  autoCapitalize="none" autoCorrect={false} keyboardType={isLogin ? "email-address" : "default"}
            ...
            {!isLogin && (
              <View style={styles.inputGroup}>
                ...
                <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.border }]}>
                  ...
                  <TextInput style={[styles.input, { color: colors.text }]} placeholder={t("أدخل البريد الإلكتروني", "Enter email")} placeholderTextColor={colors.textSecondary} value={email} onChangeText={setEmail} autoCapitalize="none" autoCorrect={false} keyboardType="email-address" testID="auth-email" />
                </View>
              </View>
            )}
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.border }]}>
                ...
                <TextInput style={[styles.input, { color: colors.text }]} placeholder={t("أدخل كلمة المرور", "Enter password")} placeholderTextColor={colors.textSecondary} value={password} onChangeText={setPassword} secureTextEntry={!showPassword} testID="auth-password" />
                <Pressable onPress={() => setShowPassword(!showPassword)} hitSlop={8}><Ionicons name={showPassword ? "eye-off-outline" : "eye-outline"} size={20} color={colors.textSecondary} /></Pressable>
              </View>
            </View>
            ...
            {!isLogin && (
              <View style={styles.inputGroup}>
                ...
                <View style={[styles.inputWrapper, { backgroundColor: inputBg, borderColor: colors.border }]}>
                  <Ionicons name="lock-closed-outline" size={20} color={colors.textSecondary} />
                  <TextInput style={[styles.input, { color: colors.text }]} placeholder={t("أعد إدخال كلمة المرور", "Re-enter password")} placeholderTextColor={colors.textSecondary} value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry={!showPassword} testID="auth-confirm-password" />
                </View>
              </View>
            )}
          ...
          <View style={styles.skipSection}>
            <Pressable onPress={() => { playSound("navigate"); router.back(); }} style={styles.skipBtn}>
              <Text style={[styles.skipText, { color: colors.textSecondary }]}>{t("متابعة بدون حساب", "Continue without account")}</Text>
            </Pressable>
          </View>
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  panelBody: { padding: 16, paddingTop: 14, gap: 10, borderTopWidth: 1 },
  panelCurrentLabel: { fontFamily: "Cairo_400Regular", fontSize: 13 },
  panelBtn: { alignItems: "center", justifyContent: "center", paddingVertical: 11, borderRadius: 10, marginTop: 2 },
  ...
  inputWrapper: { flexDirection: "row", alignItems: "center", gap: 10, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 11 },
  ...
  errorText: { fontFamily: "Cairo_400Regular", fontSize: 13, color: "#EF4444" },
