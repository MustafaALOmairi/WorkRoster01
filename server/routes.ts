...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/oauth/google", async (req: Request, res: Response) => {
    try {
      ...
      if (accessToken) {
        // Verify via access token — call Google userinfo endpoint
        const resp = await fetch(`https://www.googleapis.com/oauth2/v3/userinfo`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!resp.ok) { res.status(401).json({ error: "Invalid Google access token" }); return; }
        const info = await resp.json() as any;
        googleId = info.sub;
        email = info.email;
        displayName = info.name;
        if (!googleId) { res.status(400).json({ error: "No user ID from Google" }); return; }
      } else if (idToken && googleClientId) {
        // Verify via ID token using google-auth-library
        ...
        // Decode without verification (dev fallback)
        const parts = idToken.split(".");
        if (parts.length < 2) { res.status(400).json({ error: "Malformed token" }); return; }
        const payload = JSON.parse(Buffer.from(parts[1], "base64").toString("utf-8"));
        googleId = payload.sub || payload.id;
        email = payload.email;
        displayName = payload.name;
        if (!googleId) { res.status(400).json({ error: "No user ID in token" }); return; }
      } else {
        res.status(400).json({ error: "No token provided" }); return;
      }

      // Check existing user by google_id
      let user = await pool.query("SELECT id, username, email FROM users WHERE google_id = $1", [googleId]);
      if (!user.rows.length && email) {
        // Check by email
        user = await pool.query("SELECT id, username, email FROM users WHERE email = $1", [email]);
        if (user.rows.length) {
      ...
      if (user.rows.length) {
        ...
        // Create new user — generate unique username from display name or email
        let base = (displayName || email || "user")
          .split("@")[0]
  ...
  // ── Apple OAuth ──────────────────────────────────────────────────────────────
  app.post("/api/auth/oauth/apple", async (req: Request, res: Response) => {
    try {
      const { identityToken, fullName, email: appleEmail } = req.body;
      if (!identityToken) { res.status(400).json({ error: "No identity token" }); return; }

      // Decode JWT payload (Apple tokens are signed — verify in production with Apple JWKS)
      const parts = identityToken.split(".");
      if (parts.length < 2) { res.status(400).json({ error: "Malformed token" }); return; }
      const payload = JSON.parse(Buffer.from(parts[1] + "==", "base64").toString("utf-8"));
      const appleId: string = payload.sub;
      const email: string | undefined = appleEmail || payload.email;
      if (!appleId) { res.status(400).json({ error: "No user ID in Apple token" }); return; }

      // Check existing by apple_id
      let user = await pool.query("SELECT id, username, email FROM users WHERE apple_id = $1", [appleId]);
      if (!user.rows.length && email) {
        user = await pool.query("SELECT id, username, email FROM users WHERE email = $1", [email]);
        if (user.rows.length) {
          await pool.query("UPDATE users SET apple_id = $1 WHERE id = $2", [appleId, user.rows[0].id]);
        }
      }
      ...
      if (user.rows.length) {
        ...
        // Create new user — derive username from fullName or email
        const nameStr = fullName
          ? `${fullName.givenName || ""}${fullName.familyName || ""}`.replace(/[^a-zA-Z0-9_]/g, "")
          : (email || "user").split("@")[0].replace(/[^a-zA-Z0-9_]/g, "");
        let base = (nameStr.slice(0, 20) || "user");
        if (base.length < 3) base = "user" + base;
        let candidate = base;
        let attempt = 0;
        while (true) {
        ...
        const result = await pool.query(
          "INSERT INTO users (username, email, apple_id) VALUES ($1, $2, $3) RETURNING id, username, email",
          [candidate, email || null, appleId]
        );
        userId = result.rows[0].id;
        username = result.rows[0].username;
        userEmail = result.rows[0].email;
